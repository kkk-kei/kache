...
import kache.support.proxy.cglib.CglibProxy;
import kache.support.proxy.jdk.JDKProxy;
import kache.support.proxy.none.NoneProxy;
...
public final class CacheProxy {
    ...
    public static <K,V> ICache<K,V> getProxy(final ICache<K,V> cache) {
        ...
        if (clazz.isInterface() || Proxy.isProxyClass(clazz)) {
            return (ICache<K,V>) new JDKProxy(cache).proxy();
        }
